...
rem version: 3.1.2.RELEASE
rem build date: 20150910194706

setlocal enabledelayedexpansion

set TCRUNTIME_VER=3.1.2.RELEASE
...
:loadSetEnv
rem This is needed to find the java version
...
set JAVA_OPTS=
if %TC_MAJOR_VERSION% == 8 set REQUIRED_VERSION_STRING=1.7
if %TC_MAJOR_VERSION% == 7 set REQUIRED_VERSION_STRING=1.6
...
if %JAVA_VERSION% == 1.6 IF %TC_MAJOR_VERSION% == 7 (
  echo Java version 6 is only supported with extended support coverage
)
...
if /i "%action%" == "start" (
  ...
  echo.
  echo   [options]
  echo     -i ^<dir^>,        - The full path to the instance directory [default:
  echo       -n ^<dir^>        current working directory]
