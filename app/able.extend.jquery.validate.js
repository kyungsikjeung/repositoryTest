...
$(function() {
	...
	*/
	...
	(function ($) {
		...
		*/
		...
		$.extend($.validator.messages, {
			...
			maxbyte: $.validator.format("__FIELD_NAME__ 항목은 {0}byte를 넘을 수 없습니다."),
			maxbyteUTF8: $.validator.format("__FIELD_NAME__ 항목은 {0}byte를 넘을 수 없습니다."),
			...
			idpattern: "__FIELD_NAME__ 항목은 영문,숫자,특수문자[-,_,(,),@,.,+]로만 입력해주세요.",
			...
			mobilephone: "__FIELD_NAME__ 항목의 휴대폰번호가 유효하지 않습니다.",
			notBlank: "__FIELD_NAME__ 항목은 빈칸을 입력할 수 없습니다.",
			normalizeString: "__FIELD_NAME__ 항목은 \\/:*?\"<>| 및 공백을 포함할 수 없습니다."
		});
	}(jQuery));
	...
	jQuery.validator.addMethod("notBlank", function(value, element) {
		return this.optional(element) || !_.isBlank(value);
	}, jQuery.validator.messages.notBlank);
	...
	jQuery.validator.addMethod("mobilephone", function(value, element) {
		return this.optional(element) || /^(010|011|016|017|018|019)-?([0-9]{3,4})-?([0-9]{4})$/.test(value);
	}, jQuery.validator.messages.mobilephone);

	jQuery.validator.addMethod("normalizeString", function(value, element) {
		return this.optional(element) || /^[^\\\/\:\<\>\*\?\| ]*$/.test(value);

	}, jQuery.validator.messages.normalizeString);

	jQuery.validator.addMethod("maxbyte", function(value, element, param) {
		var totalByte = 0;
		_.each(value, function(char){
			var ASCII = escape(char);
			if( ASCII.length == 1 ) {
				totalByte++;
			} else if( ASCII.indexOf("%u") > -1  ) {
				totalByte += 2;
			} else if( ASCII.indexOf("%") > -1 ) {
				totalByte += ASCII.length/3;
			}
		});
		return totalByte <= param;
	}, jQuery.validator.messages.maxbyte);
	//UTF-8 문자열 바이트 계산. 한글은 3Byte
	jQuery.validator.addMethod("maxbyteUTF8", function(value, element, param) {
		var b,i,c;
		if(null == _.isEmpty(value)) return 0;
		for( b=i= 0; c=value.charCodeAt(i++); b+=c>>11?3:c>>7?3:1 );
		return b <= param;
	}, jQuery.validator.messages.maxbyteUTF8);

	jQuery.validator.addMethod("idpattern", function(value, element) {
		return this.optional(element) || /^[A-Za-z0-9\x2d\x5f\x28\x29\x40\x2e\x2b*]*$/.test(value);
	}, jQuery.validator.messages.idpattern);
