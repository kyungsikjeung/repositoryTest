			...
			var showError = function(errorMap, errorList) {
				if(!_.isEmpty(errorMap)) {
					...
					_.each(errorList, function(item, index, list) {
						var fieldName = fieldNameCtx[item.element.id] || fieldNameCtx[item.element.name];
						if( $.isPlainObject(item.message)) {
							item.message = item.message[item.method].replace(/__FIELD_NAME__/, fieldName);
						} else {
							item.message = item.message.replace(/__FIELD_NAME__/, fieldName);
						}
					});
					 ...
					 *	
					 *	기존 error.message 는 String형이었지만
					 *	Object{type:message, type:message...} 형태의 message를 처리하기위해 변경
					 *	(사용자에게 좀 더 세부적인 message를 출력) 	
					 */
					var errorMsgs = _.map(errorList, function(error) { return error.message; } ).join("\n");
					alert(errorMsgs);
